...
4. **Artwork goes through `components/media/GameShots.tsx`.** The site shows
   real screenshots of the games the product runs, stored in
   `public/img/games/` at each system's native resolution and upscaled by the
   browser with `image-rendering: pixelated`. Do not resize, re-encode or
   smooth them, and do not add a player, a motion library or a 3D runtime back:
   the production dependency list is React, React DOM and Next, and it should
   stay that way.

   These frames are publisher material — SEGA, Nintendo, SNK, Sony and others —
   shown on a site that also sells hardware, which is a deliberate, reversed
   decision: an earlier revision generated original pixel-art scenes to avoid
   exactly that, and they looked amateurish beside the product. If a rights
   holder objects, swap the file in `public/img/games/`; the component knows
   only paths and intrinsic sizes, so no code has to change.
...
- Sections are server components. Only things that genuinely need browser APIs
  ...
  and the theme toggle. The screenshot grid is not one of them.
