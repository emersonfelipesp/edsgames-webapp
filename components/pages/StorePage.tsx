    ...
    <PageShell locale={locale} routeKey="store" dict={dict}>
      ...
      <Section>
            ...
            <ol className="mt-5 space-y-4">
              {page.howToOrder.map((item, index) => (
                <li key={item} className="flex gap-4">
                  <span
                    aria-hidden="true"
                    className="grid size-8 shrink-0 place-items-center rounded-lg bg-neon-amber/10 font-display text-[0.5625rem] text-neon-amber"
                  >
                    {index + 1}
                  </span>
        ...
        <Panel className="mt-6 border-neon-amber/30 bg-neon-amber/5">
          <h2 className="font-display text-sm uppercase leading-relaxed text-neon-amber">
