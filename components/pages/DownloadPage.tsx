      ...
      <Section className="border-b border-line">
        ...
        <div className="mt-8 space-y-4">
          {page.beforeYouStart.map((paragraph) => (
            <p key={paragraph} className="max-w-3xl text-base leading-relaxed text-muted sm:text-lg">
              {paragraph}
            </p>
          ))}
        </div>
      ...
      <Section className="bg-abyss">
        <div className="grid gap-6 lg:grid-cols-2">
          <Panel>
            ...
            <p className="mt-4 text-base leading-relaxed text-muted">{page.requirements}</p>
          </Panel>
          <Panel>
            <h2 className="font-display text-[0.8125rem] font-bold uppercase leading-relaxed text-neon-cyan">
              {page.needTitle}
        ...
        <Panel className="mt-6 border-danger/40 bg-danger/8">
          <h2 className="font-display text-[0.8125rem] font-bold uppercase leading-relaxed text-danger">
            {page.warningTitle}
          </h2>
          <p className="mt-4 text-base leading-relaxed text-ink">{page.warningBody}</p>
        </Panel>
      </Section>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          {page.tools.map((tool) => (
            <Panel key={tool.name} interactive className="flex flex-col gap-4">
              <div>
                <h3 className="font-display text-[0.8125rem] font-bold uppercase leading-relaxed">
                  {tool.name}
                </h3>
                <p className="mt-3 text-base text-muted">{tool.description}</p>
              </div>
            ...
            <ol className="mt-8 space-y-5">
              {page.steps.map((step, index) => (
                <li key={step} className="flex gap-4">
                  <span
                    aria-hidden="true"
                    className="grid size-8 shrink-0 place-items-center rounded-lg bg-neon-cyan/10 font-display text-[0.75rem] text-neon-cyan"
                  >
